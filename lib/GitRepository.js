...
class GitRepository {
  ...
  async getReceivePackSuccessMessage(req, commands, objects) {
    return `${fusebit}Received ${commands.length} ref${
      commands.length !== 1 ? "s" : ""
    } and ${objects.length} object${objects.length !== 1 ? "s" : ""}\n\n`;
  }
  ...
  createExpress(express) {
    ...
    router.post(
      "/git-upload-pack",
      this.authorize,
      Protocol.handlePost(this, "git-upload-pack")
    );
    router.post(
      "/git-receive-pack",
      this.authorize,
      Protocol.handlePost(this, "git-receive-pack")
    );
